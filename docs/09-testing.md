...
- `TestFullFlow`：登录 → 项目（列表分页回归）→ CSRF 拒绝无头变更 → 分类/Schema → 提示词草稿/发布 ×2/差异/回滚（草稿同步）/by-slug/渲染 → 附件（预签名 + SHA/MIME/魔数校验 + 非法拒绝）→ Skill（合法上传 + 穿越拒绝）→ 专家包（成员 + 确定性构建）→ MCP 定义/Profile/清单（工具排序）→ Token 作用域（只读不能写、read+write 看不到删除工具、write-only 无法访问 MCP、撤销立即失效）→ CLI 安装器（Skill 安装/更新/移除/恢复，MCP Profile 安装/重装保留未托管/移除）。
